    ...
    <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-5 min-h-screen flex flex-col overflow-hidden">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          {activeSections.filter(s => s.is_custom).length > 0 && (
            <span className="text-xs bg-purple-100 text-purple-800 px-2 py-1 rounded-full flex items-center">
              ...
              {activeSections.filter(s => s.is_custom).length} Custom
            </span>
          )}
        </div>
      </div>
          ...
          <SortableContext items={activeSections.map(s => s.id)} strategy={verticalListSortingStrategy}>
            <div className="space-y-3 min-h-[400px] overflow-y-auto pr-1 flex-1" style={{ maxHeight: "calc(100vh - 280px)" }}>
              {activeSections.length === 0 ? (
                ...
                // CRITICAL: Sort sections by order to ensure correct display order
                [...activeSections]
                  ...
                  .map((section, index) => (
                    <SortableSection
                      ...
                      sections={sections}
                      // CRITICAL: Explicitly pass isCustom prop
