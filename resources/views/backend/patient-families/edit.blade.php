...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Family: {{ $patientFamily->family_name }}</h1>
                <p class="text-gray-600 mt-1">
                    Update family information and assign a new head of family
                </p>
            </div>

            <!-- Back Button -->
            <a href="{{ route('backend.patient-families.index') }}"
                class="flex items-center gap-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg font-medium transition">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-medium text-blue-900">Family Details</h3>
                ...
                <div class="text-right">
                    <p class="text-sm text-blue-600">Created</p>
                    <p class="text-sm text-blue-800 font-medium">
                        {{ $patientFamily->created_at->format('M d, Y') }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow">
            <form action="{{ route('backend.patient-families.update', $patientFamily) }}" method="POST" id="family-form">
                @csrf
                @method('PUT')
                            ...
                            <div class="relative md:col-span-2">
                                ...
                                <div class="flex gap-2">
                                    <input type="text" id="patient_search"
                                        ...
                                        autocomplete="off"
                                        value="@if ($patientFamily->head) {{ $patientFamily->head->full_name }} ({{ $patientFamily->head->patient_code }}) @endif">

                                    <button type="button" id="clear-patient"
                                        class="px-3 py-2 border border-gray-300 rounded-md text-gray-600 hover:bg-gray-50 @if (!$patientFamily->head_patient_id) hidden @endif">
                                        Clear
                                    </button>
                                </div>
                                ...
                                <input type="hidden" name="head_patient_id" id="patient_id"
                                    value="{{ old('head_patient_id', $patientFamily->head_patient_id) }}">

                                <!-- Selected Patient Info -->
                                <div id="selected-patient-info"
                                    class="mt-2 @if (!$patientFamily->head) hidden @endif">
                                    <div
                                        class="flex items-center justify-between p-2 bg-blue-50 rounded border border-blue-200">
                                        <div>
                                            <span class="text-sm font-medium text-blue-700" id="selected-patient-name">
                                                @if ($patientFamily->head)
                                                    {{ $patientFamily->head->full_name }}
                                                @endif
                                            </span>
                                            <span class="text-xs text-blue-600 ml-2" id="selected-patient-phone">
                                                @if ($patientFamily->head && $patientFamily->head->phone)
                                                    • {{ $patientFamily->head->phone }}
                                                @endif
                                            </span>
                                            <span class="text-xs text-gray-500 ml-2" id="selected-patient-code">
                                                @if ($patientFamily->head)
                                                    • {{ $patientFamily->head->patient_code }}
                                                @endif
                                            </span>
                                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Set initial state if patient is selected
            const initialPatientId = patientHidden.value;
            if (initialPatientId) {
                // Disable input and show selected state
            ...
            function initializePatientSearch() {
                ...
                patientInput.addEventListener('focus', function() {
                    // Only show dropdown if input is not disabled (no patient selected)
                    if (!patientInput.disabled) {
