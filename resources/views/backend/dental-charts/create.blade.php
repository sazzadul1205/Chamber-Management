...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Add Dental Chart Record</h1>
                <p class="text-gray-600 mt-1">
                    Create a new dental chart entry with tooth and condition details
                </p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow">
            <form action="{{ route('backend.dental-charts.store') }}" method="POST" id="dental-chart-form">
                @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="date" name="chart_date" value="{{ old('chart_date', date('Y-m-d')) }}"
                                    required
                                    class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('chart_date') @enderror">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="surface"
                                    class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('surface') @enderror">
                                    <option value="">Select Surface</option>
                                    @foreach (App\Models\DentalChart::surfaces() as $key => $value)
                                        <option value="{{ $key }}"
                                            {{ old('surface') == $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="condition" required
                                    class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('condition') @enderror">
                            ...
                            <div>
                                ...
                                <select name="procedure_done"
                                    class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('procedure_done') @enderror">
                                    <option value="">Select Procedure</option>
                                    @foreach (App\Models\DentalChart::procedures() as $key => $value)
                                        <option value="{{ $key }}"
                                            {{ old('procedure_done') == $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="date" name="next_checkup" value="{{ old('next_checkup') }}"
                                    class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('next_checkup') @enderror">
                            </div>
                    ...
                    <div class="border-t pt-6">
                        ...
                        <div>
                            ...
                            <textarea name="remarks" rows="3"
                                class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('remarks') @enderror">{{ old('remarks') }}</textarea>
                        </div>
                    </div>
                ...
                <div class="px-6 pb-4 bg-gray-50 border-t border-gray-200">
                    <x-back-submit-buttons back-url="{{ route('backend.dental-charts.index') }}"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.getElementById('dental-chart-form');
            ...
            @if (old('patient_id'))
                fetchPatientById({{ old('patient_id') }});
            @endif

            @if (old('tooth_number'))
                const initialTooth = toothOptions.find(item => item.number === '{{ old('tooth_number') }}');
                if (initialTooth) {
