...
@section('content')
    <div class="space-y-6">
        ...
        <!-- Form -->
        <form action="{{ route('backend.diagnosis-codes.update', $diagnosisCode->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="status"
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
                    <option value="active" {{ old('status', $diagnosisCode->status) == 'active' ? 'selected' : '' }}>Active
                    </option>
