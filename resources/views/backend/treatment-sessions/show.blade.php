        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <div class="flex items-center gap-3 mt-1">
                    ...
                    @endphp
                    <span
                        class="px-3 py-1 text-xs font-semibold rounded-full {{ $statusColors[$treatmentSession->status] ?? 'bg-gray-100 text-gray-800' }}">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-4">
                                <!-- Treatment Info -->
                                <div>
                                    <h4 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-2">Treatment
                                    </h4>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="bg-gray-50 rounded-lg p-4">
                                    <h4 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Schedule
                                    </h4>
                                    <div class="space-y-3">
                                        <div class="flex justify-between">
                                            <span class="text-gray-600">Scheduled Date:</span>
                                            <span
                                                class="font-medium">{{ $treatmentSession->scheduled_date->format('d F, Y') }}</span>
                                        </div>
                                        @if ($treatmentSession->actual_date)
                                            <div class="flex justify-between">
                                                <span class="text-gray-600">Actual Date:</span>
                                                <span
                                                    class="font-medium">{{ $treatmentSession->actual_date->format('d F, Y') }}</span>
                                            </div>
                                        @endif
                                ...
                                <!-- Duration Info -->
                                <div class="bg-gray-50 rounded-lg p-4">
                                    <h4 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Duration
                                    </h4>
                                    <div class="space-y-2">
                                        <div class="flex justify-between">
                                            <span class="text-gray-600">Planned:</span>
                                            <span class="font-medium">{{ $treatmentSession->duration_planned }}
                                                minutes</span>
                                        </div>
                                        @if ($treatmentSession->duration_actual)
                                            <div class="flex justify-between">
                                                <span class="text-gray-600">Actual:</span>
                                                <span class="font-medium">{{ $treatmentSession->duration_actual }}
                                                    minutes</span>
                                            </div>
                                        @endif
                                    </div>
                        ...
                        @if ($treatmentSession->chair)
                            <div class="mt-6 pt-6 border-t">
                                <h4 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Dental Chair
                                </h4>
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="bg-blue-50 border border-blue-100 rounded-lg p-4">
                                            <p class="text-gray-800 whitespace-pre-line">
                                                {{ $treatmentSession->doctor_notes }}</p>
                                        </div>
                                    </div>
                                @endif
                                @if ($treatmentSession->assistant_notes)
                                    <div>
                                        ...
                                        <div class="bg-green-50 border border-green-100 rounded-lg p-4">
                                            <p class="text-gray-800 whitespace-pre-line">
                                                {{ $treatmentSession->assistant_notes }}</p>
                                        </div>
                                    </div>
                                @endif
                            </div>
                ...
                <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                    ...
                    <div class="p-4 space-y-3">
                        @if ($treatmentSession->status == 'scheduled')
                            <form action="{{ route('backend.treatment-sessions.start', $treatmentSession) }}"
                                method="POST">
                                @csrf
                                <button type="submit"
                        ...
                        @if ($treatmentSession->status == 'in_progress')
                            <form action="{{ route('backend.treatment-sessions.complete', $treatmentSession) }}"
                                method="POST">
                                @csrf
                                <button type="submit"
                        ...
                        @if (in_array($treatmentSession->status, ['scheduled', 'in_progress']))
                            <!-- Postpone -->
                            <button type="button" onclick="openPostponeModal()"
                            ...
                            <!-- Cancel -->
                            <form action="{{ route('backend.treatment-sessions.cancel', $treatmentSession) }}"
                                method="POST" onsubmit="return confirm('Are you sure you want to cancel this session?')">
                                @csrf
                                <button type="submit"
                        ...
                        <!-- View Treatment -->
                        <a href="{{ route('backend.treatments.show', $treatmentSession->treatment_id) }}"
                            class="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-lg py-3 font-medium flex items-center justify-center gap-2">
                            @include('partials.sidebar-icon', [
                                'name' => 'Treatment',
                                'class' => 'w-5 h-5',
                            ])
                        ...
                        <!-- Edit Session -->
                        <a href="{{ route('backend.treatment-sessions.edit', $treatmentSession) }}"
                            class="w-full bg-gradient-to-r from-yellow-600 to-orange-600 hover:from-yellow-700 hover:to-orange-700 text-white rounded-lg py-3 font-medium flex items-center justify-center gap-2">
                            @include('partials.sidebar-icon', ['name' => 'B_Edit', 'class' => 'w-5 h-5'])
                            Edit Session
                        </a>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                    ...
                    <div class="p-6 space-y-5">
                        @if ($treatmentSession->cost_for_session)
                            <div
                                class="flex justify-between items-center p-4 bg-emerald-50 rounded-lg border border-emerald-100">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-emerald-600">Total for this session</p>
                                    </div>
                                </div>
                                <span class="text-xl font-bold text-emerald-800">৳
                                    {{ number_format($treatmentSession->cost_for_session, 2) }}</span>
                            </div>

                            <div
                                class="flex justify-between items-center p-4 bg-gray-50 rounded-lg border border-gray-200">
                                <div class="text-sm font-medium text-gray-700">Total Paid</div>
                                <div class="text-lg font-bold text-green-700">৳
                                    {{ number_format($treatmentSession->getTotalPaidAmount(), 2) }}</div>
                            </div>

                            <div
                                class="flex justify-between items-center p-4 bg-gray-50 rounded-lg border border-gray-200">
                                <div class="text-sm font-medium text-gray-700">Remaining</div>
                                <div
                                    class="text-lg font-bold {{ $treatmentSession->getRemainingAmount() > 0 ? 'text-red-700' : 'text-green-700' }}">
                                    ৳ {{ number_format($treatmentSession->getRemainingAmount(), 2) }}
                                </div>
                            </div>
                        @endif
                    </div>
                            ...
                            <div class="relative">
                                ...
                                <div class="ml-2">
                                    ...
                                    <p class="text-xs text-gray-500">
                                        {{ $treatmentSession->created_at->format('d F, Y H:i') }}</p>
                                    <p class="text-xs text-gray-400">By:
                                        {{ $treatmentSession->creator->name ?? 'System' }}</p>
                                </div>
                            </div>
                                ...
                                <div class="relative">
                                    ...
                                    <div class="ml-2">
                                        ...
                                        @if ($treatmentSession->updated_at)
                                            <p class="text-xs text-gray-500">
                                                {{ $treatmentSession->updated_at->format('d F, Y H:i') }}</p>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="relative">
                                    ...
                                    <div class="ml-2">
                                        ...
                                        <p class="text-xs text-gray-500">
                                            {{ $treatmentSession->updated_at->format('d F, Y H:i') }}</p>
                                        <p class="text-xs text-gray-400">By:
                                            {{ $treatmentSession->updater->name ?? 'System' }}</p>
                                    </div>
                                </div>
                            @endif
                        </div>
            ...
            <div class="bg-white rounded-lg shadow-xl w-full max-w-md p-6 relative">
                ...
                <form action="{{ route('backend.treatment-sessions.postpone', $treatmentSession) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="new_date" required class="w-full border rounded px-3 py-2"
                            min="{{ date('Y-m-d') }}">
                    </div>
