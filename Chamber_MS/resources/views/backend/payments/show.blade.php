...
@section('content')
    <div class="space-y-6">
        ...
        <!-- Status Banner -->
        <div class="p-4 rounded-md {{ $payment->status == 'completed' ? 'bg-green-50 border border-green-200' :
    ($payment->status == 'cancelled' ? 'bg-red-50 border border-red-200' :
        ($payment->status == 'refunded' ? 'bg-orange-50 border border-orange-200' :
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    @endphp
                    <svg class="w-6 h-6 {{ $payment->status == 'completed' ? 'text-green-600' :
    ($payment->status == 'cancelled' ? 'text-red-600' :
        ($payment->status == 'refunded' ? 'text-orange-600' : 'text-yellow-600')) }}"
                         fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                              d="{{ $statusIcons[$payment->status] ?? 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' }}" />
                    </svg>
                    <div>
                        <h3 class="text-lg font-semibold {{ $payment->status == 'completed' ? 'text-green-800' :
    ($payment->status == 'cancelled' ? 'text-red-800' :
        ($payment->status == 'refunded' ? 'text-orange-800' : 'text-yellow-800')) }}">
                            Payment {{ ucfirst($payment->status) }}
                        </h3>
                        <p class="text-sm {{ $payment->status == 'completed' ? 'text-green-700' :
    ($payment->status == 'cancelled' ? 'text-red-700' :
        ($payment->status == 'refunded' ? 'text-orange-700' : 'text-yellow-700')) }}">
                            @if($payment->status == 'completed')
                                    ...
                                    <div class="flex items-center gap-2">
                                        @php
                                            $methodIcons = [
                                                ...
                                                'mobile_banking' => 'M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z',
    ...
    ($payment->payment_type == 'partial' ? 'bg-yellow-100 text-yellow-800' :
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            @if($payment->invoice)
                                ...
                                ($payment->invoice->status == 'partial' ? 'bg-yellow-100 text-yellow-800' :
                                    ($payment->invoice->status == 'overdue' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800')) }}">
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            ...
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Allocated To</th>
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Amount</th>
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray500">Created By</th>
                                        </tr>
                                    </thead>
                                            ...
                                            <tr class="hover:bg-gray-50">
                                                ...
                                                <td class="px-4 py-2">
                                                    @if($allocation->treatmentSession)
                                                        <div class="font-medium">Session #{{ $allocation->treatmentSession->session_number }}</div>
                                                        <div class="text-xs text-gray-500">{{ $allocation->treatmentSession->session_title }}</div>
                                                    @elseif($allocation->treatmentProcedure)
                                                        <div class="font-medium">{{ $allocation->treatmentProcedure->procedure_name }}</div>
                                                        <div class="text-xs text-gray-500">{{ $allocation->treatmentProcedure->procedure_code }}</div>
                                                    @elseif($allocation->installment)
                                                        <div class="font-medium">Installment</div>
                                                        <div class="text-xs text-gray-500">Due: {{ $allocation->installment->due_date->format('d/m/Y') }}</div>
                                                    @else
                    ...
                    <div class="p-6 space-y-3">
                        ...
                        @if($payment->status == 'completed' && $payment->invoice && !$payment->installment_id)
                            <button onclick="openAllocateModal()"
                                ...
                                @include('partials.sidebar-icon', ['name' => 'B_Link', 'class' => 'w-4 h-4'])
                                Allocate to Installment
                            </button>
                        @endif
    ...
    <script>
        ...
        [document.getElementById('refundModal'), 
         ...
         document.getElementById('allocateModal')].forEach(modal => {
            if (modal) {
