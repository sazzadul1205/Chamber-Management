    ...
    <div class="space-y-6">
                                            ...
                                            <td class="px-4 py-3">
                                                <div class="flex items-center space-x-2">
                                                    ...
                                                    @if($procedure->status == 'in_progress')
                                                        <form action="{{ route('backend.treatment-procedures.complete', $procedure) }}"
                                                            method="POST" class="inline">
                                                            @csrf
                                                            <button type="submit"
                                                                class="inline-flex items-center px-2 py-1 text-xs font-medium text-green-600 bg-green-50 rounded hover:bg-green-100 transition-colors"
                        ...
                        @foreach($treatment->sessions->sortBy('session_number') as $session)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @endphp
                                    <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $statusColors[$session->status] ?? 'bg-gray-100 text-gray-800' }}">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        @if($session->status == 'scheduled')
                                            <form action="{{ route('backend.treatment-sessions.start', $session) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit"
                                                        class="inline-flex items-center px-2 py-1 text-xs font-medium text-green-600 bg-green-50 rounded hover:bg-green-100 transition-colors"
                                                        title="Start Session">
                                        ...
                                        @if($session->status == 'in_progress')
                                            <form action="{{ route('backend.treatment-sessions.complete', $session) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit"
                                                        class="inline-flex items-center px-2 py-1 text-xs font-medium text-green-600 bg-green-50 rounded hover:bg-green-100 transition-colors"
                                                        title="Mark Complete">
        ...
        @else
            <div class="p-8 text-center">
                ...
                <p class="text-gray-500 mb-4">Add sessions to schedule dental appointments for this treatment</p>
                @if($treatment->canAddSession())
                    <a href="{{ route('backend.treatments.sessions.create', $treatment) }}"
